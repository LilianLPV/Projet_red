...
func DisplayIntro(nom string) {
	time.Sleep(600 * time.Millisecond)
	utils.SlowPrint("Voici ton histoire, "+nom, 50*time.Millisecond)
...
func ReadStartChoice() string {
	return utils.ReadChoice("Ton choix : ", []string{"1", "2"})
}

func DisplayStartQuest() {
	utils.SlowPrint("Tu resserres ton équipement.", 40*time.Millisecond)
	utils.SlowPrint("Le voyage commence.", 60*time.Millisecond)
}

func DisplayPassiveRemaining() {
...
func ReadMonasteryChoice() string {
	return utils.ReadChoice("Ton choix : ", []string{"1", "2"})
}

func DisplayScene0EnterMonastery(nom string) {
	...
	utils.SlowPrint("Enchanté, "+nom, 60*time.Millisecond)
...
func DisplayScene0MonkDefeated() {
...
func DisplayScene3Intro(nom string) {
	...
	utils.SlowPrint("Sora se lève. Il vous dit : \"Félicitations pour être arrivé jusqu'ici, "+nom+".\"", 60*time.Millisecond)
	utils.SlowPrint("\"Je vais t'enseigner une dernière leçon.\"", 60*time.Millisecond)
}

func DisplayScene3Victory() {
